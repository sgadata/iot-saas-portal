...
import { Zap, Leaf, TrendingDown, Lightbulb, ArrowUpRight, BarChart3, FileDown, ShieldCheck } from 'lucide-react';
...
export default function EnergyDashboard() {
  ...
  // Referencia para la captura del PDF
  ...
  const handleExport = async () => {
    if (!window.jspdf || !window.html2canvas) {
        alert("Preparing PDF engine... please wait 1 second and try again.");
        return;
    }
    ...
    try {
      ...
      // Capturar el DOM como imagen de alta resolución
      const canvas = await window.html2canvas(element, {
        scale: 2, 
        useCORS: true,
        backgroundColor: '#0a0a0a', // Fondo oscuro premium
        logging: false
      });
      ...
      // Ajustar dimensiones del PDF (A4 tiene ~210mm ancho)
      const pdfWidth = pdf.internal.pageSize.getWidth();
      const imgProps = pdf.getImageProperties(imgData);
      const pdfHeight = (imgProps.height * pdfWidth) / imgProps.width;

      // Branding SGA DATA Oficial
      pdf.setProperties({ title: 'SGA Energy Audit' });
      
      // Rectángulo de cabecera azul premium
      pdf.setFillColor(0, 31, 63); // SGA Blue
      pdf.rect(0, 0, 210, 45, 'F');

      pdf.setFontSize(26);
      pdf.setTextColor(59, 130, 246); // Accent blue
      pdf.text('SGA DATA', 15, 25);
      
      pdf.setFontSize(14);
      pdf.setTextColor(255, 255, 255);
      pdf.text('EXECUTIVE ENERGY AUDIT REPORT', 15, 35);
      
      pdf.setFontSize(10);
      pdf.text(`ISSUED ON: ${new Date().toLocaleString().toUpperCase()}`, 130, 35);

      // Insertar la captura del dashboard
      pdf.addImage(imgData, 'PNG', 0, 45, pdfWidth, pdfHeight);
      
      // Pie de página (Safety mark)
      pdf.setFontSize(8);
      pdf.setTextColor(150, 150, 150);
      pdf.text('CONFIDENTIAL - FOR AUTHORIZED CORPORATE USE ONLY. Powered by SGA DATA Analytical Engine.', 15, 285);

      pdf.save(`SGA_Energy_Audit_${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div ref={reportRef} style={{ display: 'flex', flexDirection: 'column', gap: '2rem', padding: '10px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            <div style={{ padding: '8px 16px', background: 'rgba(59, 130, 246, 0.1)', borderRadius: '20px', border: '1px solid rgba(59, 130, 246, 0.2)', color: 'var(--accent-primary)', fontSize: '0.875rem', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <ShieldCheck size={16} /> {t('energy.efficiencyScore')}: {stats.efficiencyScore}%
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem', minHeight: '400px' }}>
            ...
            <div style={{ background: 'var(--bg-secondary)', padding: '24px', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <button 
                ...
                style={{ marginTop: 'auto', background: 'rgba(59, 130, 246, 0.1)', color: 'var(--accent-primary)', border: 'none', padding: '12px', borderRadius: '8px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s', opacity: isExporting ? 0.5 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}
              >
                {isExporting ? t('admin.generating') : <>{t('admin.exportBtn')} <FileDown size={18} /></>}
              </button>
            </div>
          </div>
      </div>
